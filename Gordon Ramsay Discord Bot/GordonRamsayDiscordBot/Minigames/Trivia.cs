...
namespace GordonRamsayBot.Minigames
{
    class Trivia
    {
        ...
        {
            await Context.Channel.SendMessageAsync("", false, Embed($"{user.Mention} took too long to answer.", ""));
        }
        ...
        public async Task AnswerTrivia(SocketGuildUser user, SocketCommandContext context, string input)
        {
            ...
            if (user == Player && triviaMode == "solo")
            {
                ...
                {
                    await context.Channel.SendMessageAsync("", false, Embed("Correct.", $"{GetName(user)}."));
                    ResetTrivia();
                    return;
                }
                await context.Channel.SendMessageAsync("", false, Embed($"Wrong, it is {correctAnswer.ToUpper()}.", $"{GetName(user)}"));
                ResetTrivia();
                return;
            }
            ...
            if (triviaMode == "all" && isTriviaBeingPlayed)
            {
                ...
                if (input == correctAnswer)
                {
                    await context.Channel.SendMessageAsync("", false, Embed($"Correct, {user.Mention} won!", $"{GetName(user)}"));
                    ResetTrivia();
                }
            }
        }
